...
async function fetchLocationStats(userId) {
  ...
  const visitedContinents = getVisitedContinents([...countrySet]);

  const continentRows = CONTINENTS.map((name) => ({
    name,
    visited: visitedContinents.has(name),
  }));
  ...
  return {
    ...
    continents: {
      visited: visitedContinents.size,
